...
/*   Updated: 2022/11/18 16:23:52 by scorreia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(const std::string name, const std::string target, const unsigned int grsign, const unsigned int exec): sign(false), name(name), target(target), grsign(grsign), exec(exec) {
	std::cout << "Constructor Form" << std::endl;
	if (grsign < 1 || exec < 1)
